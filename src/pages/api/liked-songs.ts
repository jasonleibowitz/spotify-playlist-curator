...
const fetchTrackDetails = async (song: Song): Promise<any> => {
	const response = await axios.get<TrackDetailResponse>(`https://ws.audioscrobbler.com/2.0/?method=track.getInfo&api_key=${process.env.LAST_FM_API_KEY}&artist=${encodeURI(song.artists)}&track=${encodeURI(song.name)}&format=json`);
